import './FoodFavoritesItems.scss';
...
import {getInfoFood,delFavorite} from '../FoodSearch/FoodSlice' 
...
const FoodFavoritesList=()=>{

const {loading,favoriteData}=useSelector(store=>store.food)

const content =loading?<Spinner className='Spinner' animation="border" /> : <FoodItems  data={favoriteData} />;
...
const FoodItems=({data})=>{
  const elements =data.length? data.map((item,index) => {
    return(
      <FoodItem
        key={index}
        {...item}
  ...
  }):<div className='Food__Empty'>Нет Данных</div>
  return(
...
const FoodItem=(props)=>{
  ...
  const changeLike=()=>{
     ...
     dispatch(delFavorite(props.recipe)) 
     setLike(!like)
  }
  return(
...
export default FoodFavoritesList;